...
import { Inbox, Phone, Radio, Menu, X, LogOut } from "lucide-react"

const navLinks = [
  { label: "Inbox", href: "/va/inbox", icon: Inbox },
  { label: "Calls", href: "/va/calls", icon: Phone },
  { label: "My Status", href: "/va/status", icon: Radio },
]
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-[#e6dbc9] px-3 py-3">
        <Link
          href="/portal/inbox"
          className="flex items-center gap-2 rounded-sm px-3 py-2 text-xs text-[#aaaaaa] transition-colors hover:text-[#555555]"
        >
          ← Client Portal
        </Link>
      </div>
      ...
      <div className="border-t border-[#e6dbc9] px-4 py-4">
        <div className="mb-3 px-1">
          <p className="truncate text-sm font-medium text-[#222222]">
            {user?.fullName ?? user?.firstName ?? "VA"}
          </p>
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 top-0 z-40 flex h-14 items-center border-b border-[#e6dbc9] bg-white px-4 lg:hidden">
        ...
        <Link
          href="/va/inbox"
          className="ml-3 text-lg leading-none tracking-tight text-[#222222] [font-family:var(--font-heading)]"
        >
          VA Workspace
        </Link>
      </div>
